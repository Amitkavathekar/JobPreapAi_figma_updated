...
export interface NotificationSettingItem {
  id: string;
  category: 'monetization' | 'support' | 'system';
  title: string;
  description: string;
  enabled: boolean;
...
const INITIAL_SETTINGS: NotificationSettingItem[] = [
  {
    ...
    description:
      'Send automated expiry notice 3 days prior to membership plan expiration.',
    enabled: true,
    channel: 'email',
  },
  {
    ...
    description:
      'Notify eligible candidates when special discount coupons (e.g. FESTIVE25, STUDENT50) are active.',
    enabled: true,
    channel: 'website',
  },
  {
    ...
    description:
      'Notify candidate immediately when support team posts a reply to their query.',
    enabled: true,
    channel: 'email',
  },
  {
    ...
    description:
      'Broadcast planned system upgrade window to active candidates 24h prior.',
    enabled: true,
    channel: 'website',
  },
];
...
export default function NotificationSettingsScreen() {
  const [settings, setSettings] =
    useState<NotificationSettingItem[]>(INITIAL_SETTINGS);
  const [activeTab, setActiveTab] = useState<
    'all' | 'monetization' | 'support' | 'system'
  >('all');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        <div>
          <h3
            style={{ margin: 0, fontSize: 20, fontWeight: 800, color: 'white' }}
          ></h3>
          <p style={{ margin: '4px 0 0', fontSize: 12, color: '#94a3b8' }}></p>
        </div>
      </div>
            ...
            >
              ...
              >
                ...
                >
                  {item.channel === 'email'
                    ? '📧 Email'
                    : '🌐 Website User Module Notification'}
                </span>
              </div>
            </div>
          ))}
